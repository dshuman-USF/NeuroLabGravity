...
c*************************************************************
c       newplot8v3.f - fix some residual bugs left by jj in v2.
c       bgl 8/27/95

        SUBROUTINE plot_op3(fildes,idisary,ixrange,iyrange,title,
     +  N_,loop,iopt,iline,inwary,sl_max,xc_sec,prsch,
...
c******************************************************
        if (prsch .eq. 'y') then
           ...
           do i=256,1,-1
              ...
              call move2d(fildes,0.74,abottom)
              call draw2d(fildes,0.74,atop)
              atop=atop - 0.0035
              abottom=abottom - 0.0035
           end do
           call label_cs(fildes,vrange,sl_max,itxtcl)
        end if

        ixp=1
        iyp=1           

        call labels(fildes,icell1,icell2,irow,ixrange,
     +  yc,ixp,iyp,istyc,istpyc,'a',itxtcl)
        call make_picture_current(fildes)        
        call stitle(fildes,title,xc_sec,itxtcl)
c       Query user for more screens
c       ******************************************************
10027   format (2x,'<CR> to continue..')
           print 10027
           read (*,'(A)') inst6
        call clear_control(fildes,CLEAR_DISPLAY_SURFACE)
        call clear_view_surface(fildes)
           istyc=istpyc+1
           istpyc=MIN0(istpyc+is_size,iyrange)
c          if (istpyc .le. istyc) icont = 0
          if (istyc .gt. istpyc) goto 19009
        goto 25002
c**************************************************** 
19009   call clear_control(fildes,CLEAR_DISPLAY_SURFACE)
        call clear_view_surface(fildes)
...
c       SUBROUTINE stitle
c*******************************************************
        subroutine stitle(fildes,title,xc_sec,itxtcl)
...
c*******************************************************
        subroutine labels(fildes,icell1,icell2,irow,ixrange,
...
c*******************************************************
c       SUBROUTINE label_cs
c          labels color scheme at right with min and max values
c*******************************************************
        subroutine label_cs(fildes,vrange,sl_max,itxtcl)
