...
c       *  for the first 32 entries in the color table         *
c       ********************************************************
c
c
c
...
c
      ...
      COMMON R(256),G(256),B(256)
      include 'config.defs'
      call line_color (fildes,R(i),G(i),B(i))
      end

      SUBROUTINE background_color_idx(fildes,i)
      COMMON R(256),G(256),B(256)
      include 'config.defs'
      call background_color (fildes,R(i),G(i),B(i))
      end

      SUBROUTINE text_color_idx(fildes,i)
      COMMON R(256),G(256),B(256)
      include 'config.defs'
      call text_color (fildes,R(i),G(i),B(i))
      end

      SUBROUTINE fill_color_idx(fildes,i)
      COMMON R(256),G(256),B(256)
      include 'config.defs'
      call fill_color (fildes,R(i),G(i),B(i))
      end
