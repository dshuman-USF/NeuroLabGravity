...
c
        ...
        if ((aryin(j,ny).eq.empty).and.(aryin2(j,m).eq.empty)) then
        null=null+1
        end if
        if ((aryin(j,ny).gt.empty).and.(aryin2(j,m).gt.empty))then
        hit=hit+1
        twc= (ABS(aryin(j,ny)-aryin2(j,m)))/aryin(j,ny)
        if (twc.le.thresh3) nexr=nexr+1
        end if 
        if ((aryin(j,ny).eq.empty).and.(aryin2(j,m).gt.empty)) then
        ixtra=ixtra+1
        end if
4200    continue
c
...
c       *********************************************
        if ((perxt.eq.0.0).and.(perhits.gt.0.99).and.(perwir.gt.t4)) 
...
c       *************************************************
        ...
        write (*, 40912) jaz,jsig
10000   continue
c       **************************************************
c       END OF BIG SIGNIFICANCE TEST ARRAY FILLING LOOP
c       **************************************************
40912   format (2x,I4,' of',I4,' control cycles done.')
c       **************************************************
c       END OF BIG SIGNIFICANCE TEST ARRAY FILLING LOOP..
...
3152    format (/,2x,'REMOVE *.spk *.txt & *.fwk files from directory')
        print 3151
3151    format (/,2x,'befor choosing option 1 - to avoid overwrite.')
