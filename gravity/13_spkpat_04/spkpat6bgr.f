...
c       **************************************************
      call readx (ifnam,ofnam,iarnum,aryin2,inc,NZ,
     ...
     +     imask,maskop,nomsk,sclfac)
      ...
      izz = iarnum
      call ranz(iseed,lastc,izz,aryin,aryin2)
c       **************************************************
...
c
 ...
 8039 format (2x,'Generated by program: spkpat6bgr')
...
c       *******************************************************************
c
      subroutine readx  (ifnam,ofnam,iarnum,aryin2,inc,NZ,
     +     lastc,T1,T2,T3,T4,ihard,ijmp,iopt,ismooth,thrnul,tmpmax,
     +     imask,maskop,nomsk,sclfac)
c
...
c
      print 51
 51   format (2x,'BEWARE: shuffles input BEFORE analysis!')
c       
c       
      print 100
      read (*,'(A)') IFNAM
      ...
      do 3000 i=1,iarnum
         ...
         do 4000 j=1,ncol,inc
            ...
            do 4500 ka=j,j+inc-1
               tmpka=temp(i,j)
               aryin2(i,iz)=AMAX1(tmpka,aryin2(i,iz))
 4500       continue
c       next line nulls small values defined by thrnul
            if (aryin2(i,iz).le.(thrnul*tmpmax))
     +           aryin2(i,iz)=0.0
 4000    continue
 3000 continue
c
