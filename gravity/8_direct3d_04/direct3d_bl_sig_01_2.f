...
c
c       version that also plots sig cloud: "last" points from each of 100 shifted data sets
c       *******************************************************************************
        INCLUDE 'head_04.def'
        include 'config.defs'
...
c
        ...
        real camera(13)
        real grad,sign
        character*30 DIRFILE,reply,orgreply
        character*1 inst
...
c       now scan last 'point' coordinates of each of 100 control sets
        do jz=1,NCFR
...
c     "cloud" of 100 final points for all 100 control datasets!!
...
c
        ...
        orgreply = reply
        if((reply.eq.'y').or.(reply.eq.'Y').or.reply.eq.'s')then 
c
c
...
c       ****************************************
      if (orgreply.eq.'s') then
        call mode(0)
1290    format(2x, '<cr> to step, q to quit')
        print 1290
        read(*,'(A)')reply
        if ((reply.eq.'q').or.(reply.eq.'Q')) then
         continue 
        end if
      else
        time2=0.0
